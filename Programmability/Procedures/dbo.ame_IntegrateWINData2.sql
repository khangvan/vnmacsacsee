...
SET ANSI_NULLS OFF
GO
CREATE PROCEDURE [dbo].[ame_IntegrateWINData2]
@begintlid int,
@endtlid int,
@beginstlid int,
@endstlid int
...
from [wincdata].[dbo].testlog WHERE  TL_ID >= @begintlid and TL_ID <= @endtlid and TL_ID not in
...
from [wincdata].[dbo].subtestlog WHERE  STL_ID >= @beginstlid and STL_ID <= @endtlid and  STL_ID not in
...
Units,
 Comment
from [wincdata].[dbo].subtestlog where
STL_ID > @beginstlid and STL_ID < @endstlid
and STL_ID
not in ( select [wincdata].[dbo].subtestlog.STL_ID from subtestlog 
inner join [wincdata].[dbo].subtestlog on [wincdata].[dbo].subtestlog.acs_serial = subtestlog.acs_serial 
and [wincdata].[dbo].subtestlog.station=subtestlog.station 
and [wincdata].[dbo].subtestlog.subtest_name = subtestlog.subtest_name 
and [wincdata].[dbo].subtestlog.test_id = subtestlog.test_id 
where [wincdata].[dbo].subtestlog.STL_ID > @beginstlid and [wincdata].[dbo].subtestlog.STL_ID < @endstlid )
GO