...
SET ANSI_NULLS OFF
GO
CREATE PROCEDURE [dbo].[ame_LoadDailyWINData]
@begintlid int,
@beginstlid int
 AS
...
from dailywintestlog WHERE  TL_ID >= @begintlid  and TL_ID not in
...
from dailywinsubtestlog WHERE  STL_ID >= @beginstlid and  STL_ID not in
...
Units,
 Comment
from dailywinsubtestlog where
STL_ID > @beginstlid 
and STL_ID
not in ( select dailywinsubtestlog.STL_ID from subtestlog 
inner join dailywinsubtestlog on dailywinsubtestlog.acs_serial = subtestlog.acs_serial 
and dailywinsubtestlog.station=subtestlog.station 
and dailywinsubtestlog.subtest_name = subtestlog.subtest_name 
and dailywinsubtestlog.test_id = subtestlog.test_id 
where dailywinsubtestlog.STL_ID > @beginstlid  )
GO