...
amevn_create_TestReport '05/25/2016' ,'05/26/2016'
*/
       -- =============================================
       CREATE PROCEDURE [dbo].[amevn_create_TestReport]
...
BEGIN
SET NOCOUNT off
IF OBJECT_ID('tempdb..##tempPF') IS NOT NULL/*Then it exists*/   DROP TABLE ##tempPF
...
,      SAP_Model
	...
	SELECT * INTO ##tempPF FROM tbCleanData tcd
	/*-------------------------------
	drop table ##tempPF
	select * from ##tempPF
	select * from testlog
	...
	*/-----------------------------
	...
	IF OBJECT_ID('tempdb..##TempSumByShift') IS NOT NULL
     /*Then it exists*/
        DROP TABLE ##TempSumByShift
 ...
 HourRun,
	...
	SELECT * INTO ##TempSumByShift FROM tbSumbyShift

	
	
	;WITH  tbSumDay
	AS (select daterun,station, sap_model, HourRun, TotalPass from ##TempSumByShift tsbsByShift)
...
;WITH  tbSumShift
	AS (select daterun,station, sap_model, ShiftRun, TotalPass from ##TempSumByShift tsbsByShift)
