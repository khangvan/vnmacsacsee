...
SET ANSI_NULLS OFF
GO
CREATE PROCEDURE [dbo].[ame_IntegratePANData] AS
...
from [panacsee].[dbo].testlog WHERE Test_ID not in
( select Test_ID from testlog)
...
from [panacsee].[dbo].subtestlog WHERE Test_ID not in
( select Test_ID from subtestlog)
...
select @newlastPANID = max(TL_ID) from [pancacsee].[dbo].testlog
update PANWINCounters set LastPAN_ID = @newlastPANID





select @newlastPANSTLID = max(STL_ID) from [pancacsee].[dbo].[subtestlog]
