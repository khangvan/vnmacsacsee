...
GO
CREATE PROCEDURE [dbo].[ame_getACSCountryOfOriginNew2]
@acsserial char(20),
@sapmodel char(20)
 AS
set nocount on
declare @pscserial char(20)
...
select @pscserial= psc_serial from assemblies where acs_serial = @acsserial

if @pscserial is null
begin
   select @pscserial = psc_serial from [ACSEEState].[dbo].loci where acs_serial = @acsserial
end
--print @pscserial
...
begin
select 	top 1 @countryofOrigin =rtrim(COF_Country)   ,
	@shortcof = rtrim(COF_ShortCountry) 
             from COFOrigin
where  substring(@sapmodel,1,len(COFOrigin.COF_PSC_ScannedModel)) = COFOrigin.COF_PSC_ScannedModel
order by len(COF_PSC_ScannedModel) desc
end
