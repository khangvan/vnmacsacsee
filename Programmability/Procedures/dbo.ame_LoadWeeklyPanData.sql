...
GO
CREATE PROCEDURE [dbo].[ame_LoadWeeklyPanData] 
@begintlid int,
@beginstlid int 
AS
set nocount on
...
from weeklypantestlog WHERE  TL_ID >= @begintlid and TL_ID not in
...
from weeklypansubtestlog WHERE STL_ID >= @beginstlid  and STL_ID < ( @beginstlid + 10000 ) and STL_ID not in
...
select @minweeklystlid = min(STL_ID) from weeklypansubtestlog
select @maxweeklystlid = max(STL_ID) from weeklypansubtestlog
...
exec ame_insertbulkpanweekly @minweeklystlid, @maxweeklystlid
/*
