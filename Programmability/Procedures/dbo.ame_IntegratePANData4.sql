...
SET ANSI_NULLS OFF
GO
CREATE PROCEDURE [dbo].[ame_IntegratePANData4] 
@begintlid int,
@endtlid int,
@beginstlid int,
@endstlid int 
...
from [pandata].[dbo].testlog WHERE  TL_ID >= @begintlid and TL_ID <= @endtlid and TL_ID not in
...
from [pandata].[dbo].subtestlog WHERE STL_ID >= @beginstlid and STL_ID <= @endstlid and STL_ID not in
...
Units,
 Comment
from [pandata].[dbo].subtestlog where
STL_ID > @beginstlid and STL_ID < @endstlid
and STL_ID
not in ( select [pandata].[dbo].subtestlog.STL_ID from subtestlog 
inner join [pandata].[dbo].subtestlog on [pandata].[dbo].subtestlog.acs_serial = subtestlog.acs_serial 
and [pandata].[dbo].subtestlog.station=subtestlog.station 
and [pandata].[dbo].subtestlog.subtest_name = subtestlog.subtest_name 
and [pandata].[dbo].subtestlog.test_id = subtestlog.test_id 
where [pandata].[dbo].subtestlog.STL_ID > @beginstlid and [pandata].[dbo].subtestlog.STL_ID < @endstlid )
