...
-- Define input parameters
	...
	@orttype	char(5) = NULL, -- Scanned component serial number
	@ortbin		char(10) = NULL -- Scanned component serial number
...
*/
	Begin Transaction
		...
		Values(@aserial, 'ENTERORT', @sname, @testid, 'P', ' ',
			@tdate,0)
		if @@ERROR <> 0
		   begin
		...
		Values(@aserial, @sname, 'ENTERORT', @testid, 'P', ' ',
			0,0.0,' ',@strval)
		if @@ERROR <> 0
		   begin
			--raiserror('E403.13 Serious Error. Failed to append record to assemblies db',17,1)
		...
		-- ORTVault
		Insert ORTVault (ACS_Serial, ORT_Type, ORT_Bin, EnterDate)
		Values(@aserial, @ORTType, @ORTBin, @tdate)
		if @@ERROR <> 0
		   begin
			--raiserror('E403.13 Serious Error. Failed to append record to assemblies db',17,1)
			Rollback Transaction
