...
create proc [dbo].[sp_get_down_stream]
...
AS
	...
	if @sname is NULL
	   begin
		raiserror ('E9.1 Illegal Parameter Value. You must specify a station name.',
			16,1) with nowait
		return
	   end
	...
	if not exists(select Status from Stations 
	   ...
	   begin
		raiserror ('E9.2 Illegal Parameter Value. Active station %s does not exist.',
